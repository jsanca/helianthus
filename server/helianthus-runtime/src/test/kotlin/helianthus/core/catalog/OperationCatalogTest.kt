...
/**
 * Loads the production `operations.yml` via [CatalogLoader] and exercises
 * [OperationCatalog.resolveOperation] for query/queryRef resolution,
 * configuration lookup, and unknown-id errors.
 */
class OperationCatalogTest {

    private fun loadMainCatalog(): OperationCatalog {
        val stream = OperationCatalogTest::class.java.classLoader
            .getResourceAsStream("operations.yml")
            ?: throw IllegalStateException("operations.yml not found on test classpath")
        return stream.use { CatalogLoader().load(it).operationCatalog }
    }
