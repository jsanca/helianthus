...
import helianthus.core.access.sql.NamedParameterSql
import org.slf4j.LoggerFactory

/**
 * Pipeline step that executes the operation's SQL and stores the resulting
 * [helianthus.core.result.CloseableRowStream] in the context.
 *
 * Supports both positional (`?`) and named (`:paramName`) parameter styles.
 * Named-style SQL is rewritten to positional form before execution; positional
 * SQL receives only the non-null bound parameters in declaration order.
 *
 * @param dataAccess the data-access layer used to execute the query
 * @param fetchSize JDBC fetch-size hint passed through to the data access
 */
